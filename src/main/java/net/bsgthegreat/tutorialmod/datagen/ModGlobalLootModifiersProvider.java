package net.bsgthegreat.tutorialmod.datagen;

import net.bsgthegreat.tutorialmod.TutorialMod;
import net.bsgthegreat.tutorialmod.item.ModItems;
import net.bsgthegreat.tutorialmod.loot.AddItemModifer;
import net.bsgthegreat.tutorialmod.loot.AddSusSandItemModifier;
import net.bsgthegreat.tutorialmod.util.ModTags;
...
public class ModGlobalLootModifiersProvider extends GlobalLootModifierProvider {
    ...
    protected void start() {
        add("pine_cone_from_grass", new AddItemModifer(new LootItemCondition[] {
                LootItemBlockStatePropertyCondition.hasBlockStateProperties(Blocks.GRASS).build(),
                LootItemRandomChanceCondition.randomChance(0.35f).build()}, ModItems.PINE_CONE.get()));

        add("pine_cone_from_creeper", new AddItemModifer(new LootItemCondition[] {
                new LootTableIdCondition.Builder(new ResourceLocation("entities/creeper")).build() }, ModItems.PINE_CONE.get()));

        add("metal_detector_from_jungle_temples", new AddItemModifer(new LootItemCondition[] {
